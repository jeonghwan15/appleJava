...
public class board_reply  implements board_action{
	...
	public String action(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		board org = dao.selectDetail
		
		
		
		
		// 메일 보내기
		final String username="lude1108@naver.com";
		final String password="XGG7XXXF6LHS";
		...
		try { //메시지 생성
			...
			msg.setFrom(new InternetAddress(username));//보내는 이메일
			msg.setRecipients(Message.RecipientType.TO,
					InternetAddress.parse( org.getAnswer_mail() )); //받는 이메일
			msg.setSubject(org.getTitle()+" 게시글에 답글 등록"); //제목	
			
			msg.setText(org.getTitle()+" 의 글에 "+user.getName()+"답글이 등록되었습니다."
					+"확인하시려면 아래 링크를 클릭해주세요. \n"+
					"http://192.168.0.32/board_read.do?id="+bid,
					"UTF-8"); //내용
			Transport.send(msg);
