...
//		System.out.println( stack.search("데리버거") );  // 맨 밑부터 0(새우버거)
		
		Stack<String> back = new Stack<String>();
		Stack<String> front = new Stack<String>();
		String now="네이버메인";
		
		// 브라우저에서 페이지 이동 시 스택에 저장되는 방법 구현
		
		now = pageMove(back, front, now);
	}
	
		static void pageMove(Stack<String> back, Stack<String> front, String now) {
...
// 계산기, 프로그램의 실행취소 및 되살리기, 웹브라우저의 뒤로, 앞으로 버튼 등등
